...
const Header = () => {
  ...
  return (
    <>
          ...
          <div className="hidden lg:flex lg:justify-center lg:gap-3 lg:items-center">
            {links.map((link) => (
              <Link
                href={link.href}
                key={link.href}
                className="btn btn-ghost btn-sm rounded-lg hover:bg-primary/10 transition-colors"
                title={link.label}
              >
                {link.label}
              </Link>
      ...
      {/* Moved outside header to fix z-index issues with sticky positioning */}
              ...
              <div className="flex flex-col gap-y-2 items-start">
                {links.map((link) => (
                  <Link
                    href={link.href}
                    key={link.href}
                    className="btn btn-ghost rounded-lg hover:bg-primary/10 justify-start w-full"
                    title={link.label}
                  >
                    {link.label}
                  </Link>
